...
class MambaLLM(LightningModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.embedding = nn.Embedding(num_embeddings=vocab_size, embedding_dim=d_input)
        ...
        self.llm = nn.ModuleList(
            [
                nn.ModuleList([MambaBlock(**self.mamba_par), RMSNorm(d_input)])
    ...
    def generate(
        ...
        cache = (
            None,
            torch.zeros(batch_size, d_model, ker_size - 1, device=self.device),
        )
        ...
        yield {
            int(pid): tokenizer.decode(raw, skip_special_tokens=True)
            for pid, raw in zip(pidx, inp)
        }
        ...
        while num_tokes < token_lim and len(pred):
            ...
            idxs = prob.topk(
                k=vocab_size - use_top_k, largest=False, sorted=False
            ).indices
